...
class TacGenerator:
    ...
    def gen_tac(self, node):
        ...
        if node.type == 'dclassign':
            self.write_line(c[1], '=', self.gen_tac(c[2]))
        if node.type == 'bool':
        ...
        if node.type == 'strcast':
            v = self.get_var()
            self.write_line(v, '=', 'num2str(', self.gen_tac(c[0]), ')')
            return v
        if node.type == 'str':
            return self.gen_tac(c[0])
        if node.type == 'assign':
            self.write_line(c[0], '=', self.gen_tac(c[1]))
        if node.type == 'if':
            if len(c) == 4:
        ...
        if node.type == 'while':
            ...
            self.write_line(label1)
            self.write_line('if', 'false', self.gen_tac(c[0]), 'goto', label2)
            self.gen_tac(c[1])
